...
class ConversationRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun getConversationsSupport(
            page: Int,
            toSupport: Boolean,
        ): Either<Failure, List<Conversation>> {
            return when (
                val response =
                    service.conversationsSupport(
                        token = "${SettingRemote.BEARER} ${dataStore.token()}",
                        perPage = SettingRemote.PER_PAGE,
                        page = page,
                        toSupport = toSupport,
        ...
        override suspend fun sendConversationSupport(
            ...
            return when (
                val response =
                    service.sendConversationSupport(
                        token = "${SettingRemote.BEARER} ${dataStore.token()}",
                        body =
                            SendConversationSupportRequest(
                                content = content,
                                toSupport = toSupport,
                                userIds = userIds,
                            ),
        ...
        override suspend fun getUserByCourse(
            name: String,
            courseId: Int,
        ): Either<Failure, List<User>> {
            return when (
                val response =
                    service.userByCourse(
                        token = "${SettingRemote.BEARER} ${dataStore.token()}",
                        name = name,
                        courseId = courseId,
        ...
        override suspend fun getUserByIds(
            ids: String,
            courseId: Int,
        ): Either<Failure, List<User>> {
            return when (
